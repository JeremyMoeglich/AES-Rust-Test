...
pub fn encrypt(cipher: &Key, plaintext: &str) -> Vec<u8> {
    match plaintext.find("\0") {
        Some(_) => panic!("Plaintext contains null byte"),
        None => (),
    };
...
pub fn decrypt(cipher: &Key, ciphertext: &[u8]) -> Result<String, String> {
    let round_keys = key_schedule::key_schedule(&cipher);
    ...
    let string = String::from_utf8(plaintext).expect("Invalid UTF-8");
